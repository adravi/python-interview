...
initials = {
    'o': ['one'], 't': ['two'], 'f': ['four'], 's': ['six'], 'e': ['eight'], 'n': ['nine']
}
...
def calibration_values(values):
    ...
    for val in values:
        ...
        while i < len(val):
            if val[i].isdigit() and not digit_found:
                first_digit = val[i]
                i += 1
                continue

            elif val[i] in initials:
                possible = initials[val[i]][0]
                print('possible:', possible)
                token = ''
                j = 0
                
                while j <= len(possible) and val[i] == possible[j]:
                    print(val[i], '==', possible[j])
                    token += val[i]
                    print(token)
                    j += 1
                    i += 1

                print(token, 'token confirmed')
...
print(calibration_values(input2))